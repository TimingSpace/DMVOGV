...
def pose2motion3(data,camera_pos):
    data_size = data.shape[0]
    all_motion = np.zeros((data_size-1,12))
    for i in range(0,data_size-1):
        pose_curr = line2mat(data[i,:])
        ...
        r_x,r_y,r_z = motion_eular
        motion_l = (motion[[0,2],3].T*motion[[0,2],3]).item()
        ratio = camera_pos/motion_l + 0.5
        r_y*=ratio
...
def pose2motion2(data,ratio=1,pitch=0):
    data_size = data.shape[0]
    all_motion = np.zeros((data_size-1,12))
    for i in range(0,data_size-1):
        pose_curr = line2mat(data[i,:])
...
def motion2pose3(data,camera_pos):
    data_size = data.shape[0]
    all_pose = np.zeros((data_size+1,12))
    temp = np.eye(4,4).reshape(1,16)
    all_pose[0,:] = temp[0,0:12]
...
def motion2pose2(data,ratio=1,pitch = 0):
    data_size = data.shape[0]
    all_pose = np.zeros((data_size+1,12))
    temp = np.eye(4,4).reshape(1,16)
    all_pose[0,:] = temp[0,0:12]
...
def eular2pose2(data,ratio,pitch):
    motion = eular2motion(data)
    pose = motion2pose2(motion,ratio,pitch)
    return pose

def eular2pose3(data,camera_pos):
    motion = eular2motion(data)
    pose = motion2pose3(motion,camera_pos)
    return pose
def SE2se(SE_data):
    result = np.zeros((6))
